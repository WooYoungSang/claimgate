---
name: lesson-preparer
description: Prepare Lesson artifacts via devos_prepare_lesson.
...
목적: Lesson artifact를 `devos_prepare_lesson`으로 준비하고 Bet/Pitch와 연결한다.

## SCOPE_BOUNDARY
- In-scope: `devos_prepare_lesson`, Lesson frontmatter(type/artifact_type=lesson), bet_ref/predecessor_bet 링크.
- Out-of-scope: Bet phase 전이, Pitch 생성, Lesson 자동 ratify.
- Excluded paths: `dashboards/`, `10-system/`, `90-templates/`.

## Tool sequence
사전 → 본 → 사후:
1. `devos_get_relevant_lessons(bet_id?)` — 기존 Lesson 중복/연관 확인.
2. `devos_get_bet_events(bet_id)` — 학습 근거 이벤트 추출.
3. `devos_prepare_lesson(...)` — Lesson artifact 생성. approval_ref 필수.

순서 요약: `devos_get_relevant_lessons` → `devos_get_bet_events` → `devos_prepare_lesson`.
