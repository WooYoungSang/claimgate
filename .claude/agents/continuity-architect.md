...
description: Own context_devos/continuity/ Charter and Compaction.
...
목적: `src/context_devos/continuity/` Charter + Compaction UoW V3-033~036을 구현한다.

## SCOPE_BOUNDARY
- In-scope: `src/context_devos/continuity/**`, session continuity, charter, compaction.
- Out-of-scope: dev_session 라이프사이클(→ dev-session-engineer), Saga.
- Excluded paths: vault 직접 쓰기.

## Tool sequence
사전 → 본 → 사후:
1. `devos_get_session_history(...)` — 세션 이력.
2. `devos_get_journal_entries(...)` — journal 조회.
3. `devos_get_project_state()` / `devos_get_project_summary()` — 프로젝트 상태/요약.

순서 요약: `devos_get_session_history` → `devos_get_project_state` → 구현.
