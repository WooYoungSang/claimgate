---
name: uow-author
description: Author UoW artifacts via the current devos_create_scope compatibility surface.
...
목적: UoW artifact를 `90-templates/tpl-uow.md`와 `99_constitution/vault-os.md` 기준으로 propose-only 생성한다. 현재 MCP 호환 표면은 `devos_create_scope`이며, 결과는 `artifact_type: uow`로 `20-projects/60-uow/`에 저장된다.

## SCOPE_BOUNDARY
- In-scope: `devos_create_scope`, `devos_get_scope_map`, UoW frontmatter(type/artifact_type=uow), Bet-UoW 매핑, `20-projects/60-uow/` 라우팅 검증.
- Out-of-scope: Bet/FR/NFR/ADR 생성, Bet phase 전이, UoW ready/shipped/accepted 전이, scope 외 vault 영역 수정.
...
## Tool sequence
사전 → 본 → 사후:
1. `devos_get_scope_map(bet_id?)` — 기존 UoW/scope coverage와 중복 경계 확인.
2. `devos_create_scope(...)` — 현재 compatibility create surface. `artifact_type: uow`로 생성되어야 하며, approval_ref/Safety Guards 조건을 충족해야 한다.
3. `devos_validate_shapeops_consistency(project_id?)` — Bet-UoW-FR/NFR 일관성 점검.

순서 요약: `devos_get_scope_map` → `devos_create_scope` → `devos_validate_shapeops_consistency`.

## Post-write readback gate (fail-closed)
After `devos_create_scope(...)` returns, do not report success from the write response alone. Perform an approved post-write readback of the created UoW (prefer `devos_get_uow_detail` plus a vault-file read of the returned `vault_path`; if readback is unavailable, return `BLOCK`/`HITL_REQUIRED`).
...
If any assertion fails or required evidence is missing, fail closed: report `BLOCK`/`HITL_REQUIRED`, include the failing assertion and evidence refs, and do not claim the UoW was created successfully.

## UoW creation constraints
- Use `90-templates/tpl-uow.md` semantics even though the compatibility tool name is `devos_create_scope`.
- UoW must remain a work contract under a Bet: include Bet link, scope boundary, done criteria, acceptance criteria, target files, coordination rule, and verification ladder in template-defined slots.
- Frontmatter must include `type`, `artifact_type: uow`, `project`, `id`, `title`, `phase: shaped`, and Bet reference fields required by template/schema. `phase: shaped` is the default workflow state for newly-authored UoW contracts emitted by `devos_create_scope`; never omit it or replace it with `status`.
- Validate the returned vault path and content contract: `20-projects/60-uow/`, `artifact_type: uow`, `phase: shaped`, and the parent Bet reference must all be present before reporting success.
- This author role never marks UoW ready/shipped/accepted and never waives a Gate; it only drafts/proposes the work contract.
