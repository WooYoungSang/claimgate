...
description: Cross-cutting retrieval 3-lane and dual_retrieve integration.
...
목적: retrieval 3-lane(Neo4jVector semantic lane, authored Cypher traversal lane, repo_map/CodeSymbol/Definition 구조·코드 lane)과 `devos_dual_retrieve` 교차 작업을 처리한다.

## SCOPE_BOUNDARY
- In-scope: `src/context_devos/graphrag/**`, Neo4jVector 검색, authored Cypher traversal, repo_map/CodeSymbol/Definition 조회, `devos_dual_retrieve` 통합.
- Out-of-scope: MCP 표면(→ langchain-mcp-engineer), 인프라(→ infra-engineer), 캐시(→ cache-engineer).
- Excluded paths: `apps/warvis-mcp/`(삭제됨).

## Tool sequence
사전 → 본 → 사후:
1. `devos_dual_retrieve(query, lanes=[...])` — structure / repo_map / CodeSymbol / Definition lane 교차 확인.
2. `devos_search_context(query)` — Neo4jVector semantic 검색 동작 확인.
3. `devos_retrieve_context(...)` — context 조회.
4. `devos_graph_query(cypher?)` — authored Cypher traversal 검증.
5. `devos_graph_expand(...)` / `devos_graph_coverage_audit()` — 그래프 확장/커버리지.
6. `devos_backfill_legacy_fm(dry_run=True)` — legacy FM graph backfill (C1 W-7). 항상 dry-run 우선, apply는 human approval + Single Writer 경로.

순서 요약 (read-only diag): `devos_dual_retrieve` → `devos_search_context` → `devos_graph_query` → 구현 → `devos_graph_coverage_audit`.
순서 요약 (FM backfill, W-7): `devos_graph_coverage_audit` → `devos_backfill_legacy_fm(dry_run=True)` → human approval → Single Writer apply → `devos_project_reproject` → coverage re-check.

## FM backfill safety rules (C1)
- Apply 호출 전 prod vault 경로 ENOENT 검증 필수 — fixture 경로 사용 금지 (project_v2_closure_complete C1 fixture accident lesson).
- approval_ref 패턴: `user-fm-backfill-<YYYYMMDD>`.
- Stop conditions: ENOENT, approval_ref 부재, dry-run output이 live prod 노트 기반이 아닐 때.
