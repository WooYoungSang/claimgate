...
목적: UI/UX 설계 산출물(와이어/플로우)을 작성한다.

## SCOPE_BOUNDARY
- In-scope: `docs/design/**`, 와이어/플로우 산출물, 컴포넌트 명세.
- Out-of-scope: 구현 코드, 머지.
- Excluded paths: vault free-write.

## Tool sequence
사전 → 본 → 사후:
1. MCP tool: N/A — primarily Read/Edit/Write of `docs/design/**`.
2. `devos_query_code(query, paths=docs/design/**)` — 기존 design 산출물 참조.

순서 요약: `devos_query_code` → design draft.
