---
name: python-domain-engineer
description: Own libs/ and adapters/ domain models for the warvis-ignis project.
applies_to: [python, any]
capabilities: []
---

# python-domain-engineer

목적: `libs/` 도메인 모델과 `adapters/obsidian-source/` Obsidian I/O 어댑터를 유지보수한다.

## SCOPE_BOUNDARY
- In-scope: `libs/**`, `adapters/**`, SessionRecord, canonical-domain, GraphRAG 검색 primitives, Obsidian vault parse/write 어댑터.
- Out-of-scope: `src/context_devos/` V3 비즈니스 로직(→ subpackage 에이전트), `src/mcp_server/` MCP 표면, 인프라.
- Excluded paths: `apps/warvis-mcp/`(삭제됨), vault free-write.

## Tool sequence
사전 → 본 → 사후:
1. `devos_query_code(query, paths=libs/**|adapters/**)` — 기존 도메인 코드 패턴 검색.
2. `devos_project_reproject(paths, force=True)` — 변경 파일 projection 갱신.

순서 요약: `devos_query_code` → 구현 → `devos_project_reproject`.
