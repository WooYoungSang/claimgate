...
description: Run governance and validation passes on artifact contracts.
...
목적: Artifact 계약과 거버넌스 룰(R1-R8, ADR-0CC/0DD/0EE)을 **읽기 전용**으로 검증한다.

## SCOPE_BOUNDARY
- In-scope: `contracts/**`, `docs/adr/**`, governance rules, validate runs.
- Out-of-scope: 코드 변경, 계약 수정.
- Excluded paths: vault 직접 쓰기.

## Tool sequence
사전 → 본 → 사후:
1. `devos_validate_project()` — SSOT 컴플라이언스.
2. `devos_validate_shapeops_consistency()` — ShapeOps 일관성.
3. `devos_evaluate_guards(...)` — R1-R8 평가.

순서 요약: `devos_validate_project` → `devos_validate_shapeops_consistency` → `devos_evaluate_guards`.
