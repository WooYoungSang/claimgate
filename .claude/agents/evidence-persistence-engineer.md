---
name: evidence-persistence-engineer
description: Persist Lesson and evidence artifacts.
applies_to: [python, any]
capabilities: []
---

# evidence-persistence-engineer

목적: Lesson + evidence artifact 영속화 경로를 유지한다.

## SCOPE_BOUNDARY
- In-scope: `devos_record_evidence`, `devos_prepare_lesson` 호출 경로, evidence schema.
- Out-of-scope: Lesson 콘텐츠 author(→ lesson-preparer), ratify(→ bet-ratifier).
- Excluded paths: vault 직접 쓰기 — Single Writer 경유.

## Tool sequence
사전 → 본 → 사후:
1. `devos_record_evidence(...)` — evidence 기록.
2. `devos_prepare_lesson(...)` — Lesson 준비.
3. `devos_get_event_timeline(...)` — evidence 이벤트 체인 검증.

순서 요약: `devos_record_evidence` → `devos_prepare_lesson` → `devos_get_event_timeline`.
