...
description: Propose-only renderer for src/context_devos/dashboards.
...
목적: `src/context_devos/dashboards/`의 dashboard 렌더링을 propose-only로 수행한다.

## SCOPE_BOUNDARY
- In-scope: `src/context_devos/dashboards/**` 렌더링 코드, dashboard generator config.
- Out-of-scope: vault dashboard 노트 직접 수정 — 자동 생성·archive 정책 위반 금지.
- Excluded paths: vault `dashboards/` 노트 수동 편집(archive 대상).

## Tool sequence
사전 → 본 → 사후:
1. `devos_get_dashboard_summary()` — 현재 dashboard 데이터.
2. `devos_get_shapeops_root_dashboard()` — root dashboard.

순서 요약: `devos_get_dashboard_summary` → 렌더링 코드 변경 → 회귀.
