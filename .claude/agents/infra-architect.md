...
description: Own context_devos/schema, sync, and errors subpackages.
...
목적: `context_devos/schema/`, `context_devos/sync/`, `context_devos/errors.py` 인프라 골격을 유지한다.

## SCOPE_BOUNDARY
- In-scope: `src/context_devos/schema/**`, `src/context_devos/sync/**`, `src/context_devos/errors.py`, Neo4j 스키마 DDL, SyncPipeline.
- Out-of-scope: 비즈니스 도메인 로직(다른 subpackage), MCP 표면, 인프라(`infra/`).
- Excluded paths: vault 직접 쓰기.

## Tool sequence
사전 → 본 → 사후:
1. `devos_project_reproject(paths, force=True)` — schema/sync 관련 ShapeOps projection freshness.
2. `devos_query_code(query, paths=src/context_devos/schema/**|sync/**)` — 기존 스키마 패턴.

순서 요약: `devos_project_reproject` → `devos_query_code` → 구현 → `devos_project_reproject`.
