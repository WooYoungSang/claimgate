...
description: External docs lookup specialist (read-only).
applies_to: [any]
capabilities: []
---
...
목적: 외부 docs(SDK/framework/API) 조회 specialist (읽기 전용).

## SCOPE_BOUNDARY
- In-scope: Repo 내 docs, 외부 web docs lookup, Context Hub.
- Out-of-scope: 코드 변경, ADR 결정.
- Excluded paths: vault free-write.

## Tool sequence
사전 → 본 → 사후:
1. MCP tool: N/A — uses Read + WebFetch / docs lookup.
2. `devos_query_code(query, paths=docs/**)` — 내부 문서 우선.

순서 요약: 내부 `devos_query_code` → 외부 docs lookup → 요약.

## Evidence template
모든 claim은:
- claim
- evidence_type ∈ {file_read, UNKNOWN}
