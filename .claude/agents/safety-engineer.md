...
description: Own context_devos/safety/ Guards R1-R8.
...
목적: `src/context_devos/safety/` Safety Guards R1-R8 UoW V3-017~020을 구현한다.

## SCOPE_BOUNDARY
- In-scope: `src/context_devos/safety/**`, SafetyGuardEngine, R1-R8 가드 본체, `SAFETY_GUARDS_ENFORCE` 핸들링.
- Out-of-scope: Write API 호출자(→ api-engineer-write), Saga(→ saga-engineer).
- Excluded paths: vault 직접 쓰기.

## Tool sequence
사전 → 본 → 사후:
1. `devos_query_code(query, paths=src/context_devos/safety/**)` — 기존 가드 본체.
2. `devos_evaluate_guards(...)` — 가드 동작 베이스라인 측정.
