---
name: ctrl-tower-codex-dispatcher
description: Control Tower orchestrator-side helper that prepares one UoW Codex MCP dispatch with SCOPE_BOUNDARY, AC mapping, and a return envelope contract.
applies_to: [any]
capabilities: [codex-dispatch, scope-boundary, envelope-schema, worktree-guard]
---

# ctrl-tower-codex-dispatcher

## 목적

Control Tower orchestrator 가 UoW 1건을 Codex MCP implementer 로 넘길 때
필요한 worktree, prompt, AC, evidence, return envelope 를 빠짐없이 조립한다.

## SCOPE_BOUNDARY
- In-scope: orchestrator-side prompt 조립, worktree path 검증, AC 매핑, return envelope schema 검증, `mcp__codex__codex` 호출 directive 생성.
- Out-of-scope: `src/` business logic 직접 구현, Codex subprocess 직접 실행, vault note write, Wave ratify 최종 승인.
- Excluded paths: `/data/vault/**`, `~/Workspace/warvis-vault/**`, `.omc/**` 영구 산출, origin push/PR.

## Codex standalone operating profile (Harness C)

본 dispatcher 는 Claude-host DELEGATE 경로에서 Codex MCP 호출 prompt 를 조립하는
역할이다. forge coordinator 가 이미 Codex host (`$forge` / `$forge-lite`) 에서
실행 중이면 **Codex host uses its native implementer**; 이 dispatcher/Claude-only
dispatch skill 을 통하지 않고 `.codex/codex-roles/implementer.md` 의 profile 을
직접 적용한다.

Codex-host dogfood profile (`codex_operating_profile`):

```yaml
codex_operating_profile:
  model: gpt-5.5
  reasoning_effort: medium
  verbosity: low
  tool_preamble: target result + constraint + first validation before tool-heavy work
  phase_discipline: commentary for tools/progress; final only for completed report
  context_stop_rule: stop once requested evidence is sufficient
  parallel_read_policy: batch independent reads before editing
  early_stop: action-gated early stop only on blocker/destructive boundary/failed gate
  conflict_priority:
    - safety/Vault OS/Rule 8 no-self-approval
    - operator instructions
    - Bet AEC/UoW contract
    - appetite/scope hammering
    - generated harness defaults
  persistence: local repo artifacts only; no vault write, no push, no self-ratify
  model_identity_confidence: MEDIUM
```

Per-UoW dispatch MUST either inject this `codex_operating_profile` into the
Codex MCP prompt (Claude-host delegate path) or apply it directly before local
implementation work (Codex-host native path).

Rule 8 remains invariant in both paths: dispatcher/implementer output is evidence,
not approval; cr/architect/operator contexts own review/ratify boundaries.

## Tool sequence
사전 → 본 → 사후:
1. `pwd && git log --oneline -1 && git branch --show-current` — cwd 와 HEAD 증명.
2. SSOT spec (`docs/orchestration/CODEX-MCP-PATTERN.md`, `docs/specs/claude-codex-orchestration-pattern.md`) 읽기.
3. RED 테스트 또는 검증 실패를 먼저 만든 뒤 GREEN 구현 prompt 로 이동.
4. `git diff --name-only` 로 in-scope 글로브를 벗어난 변경이 없는지 확인.
5. `python3 -m pytest tests/architecture -q` 를 최소 1회 실행.
6. return envelope 는 stdout 마지막 fenced JSON block 으로 출력.

순서 요약: `pwd` → spec read → RED prompt → GREEN prompt → arch test → envelope emit.

## AC 매핑

- AC-DISPATCH-1: prompt 가 SCOPE_BOUNDARY 6 블록 (in-scope/out-of-scope/excluded/mutation-defaults/worktree/return-envelope) 을 모두 포함한다.
- AC-DISPATCH-2: worktree path 가 absolute 이고 known workspace 아래에 있다.
- AC-DISPATCH-3: return envelope 가 draft-07 schema 와 호환된다.
- AC-DISPATCH-4: F1/F5/F8 mitigation 문구가 prompt 안에 명시된다.
...
SCOPE_BOUNDARY:
  in-scope:
    - <이 dispatch 의 in-scope path 만 나열>
  out-of-scope:
    - <인접 lane 과 persona conflict 명시>
  excluded:
    - /data/vault/**
...
worktree:
  <absolute path>
  첫 명령: pwd && git log --oneline -1

Goal: UoW slug 하나를 받아 `scripts/ctrl_tower/codex_dispatch.py` 와 동일한
SCOPE_BOUNDARY 기반 task prompt 를 산출한다. Codex implementer 는
`mcp__codex__codex` 를 통해 `developer-instructions=.claude/codex-roles/implementer.md`
로 호출된다.
AC:
  - <위 AC 매핑을 복사>
Evidence template:
...
### RED
- 실패하는 테스트 또는 검증 스텁을 먼저 기록한다.
- RED evidence 는 `test_run` 또는 `grep_match` 로 남긴다.
- RED 없이 GREEN 만 제출하면 reviewer 가 MAJOR 로 분류한다.

### GREEN
- 최소 구현 prompt 로 AC 를 통과시킨다.
- 기존 public contract 를 변경할 때는 backward-compat evidence 를 첨부한다.
- vault write 가 필요한 경우 직접 쓰지 않고 propose 문구만 남긴다.

### REFACTOR
- scope 밖 diff 를 제거한다.
- 중복 prompt 또는 schema drift 를 정리한다.
...
- F1: danger-full-access 누락을 prompt 단계에서 차단한다.
- F2: 직접 `codex exec` 대신 Agent wrapper / `mcp__codex__codex` 문구를 제시한다.
- F5: `mutate_vault: false` 를 고정한다.
- F7: in-scope glob 교차를 사전에 확인한다.
- F8: DO NOT push/PR 문구를 반복한다.

## Sandbox 결정 (operator-acknowledged)

본 dispatcher 는 항상 `sandbox=danger-full-access` 를 사용한다. 근거:

- `workspace-write` 는 `.git/worktrees/<sg>/index.lock` (git worktree metadata 가 worktree dir 외부에 존재) 을 차단해 codex MCP 가 worktree 안에서 commit 을 만들 수 없다.
- 2026-05 시점 codex MCP 에 알려진 우회 경로 없음.
- 완화 수단: (a) SG 별 worktree branch 격리, (b) operator-gated push, (c) prompt 의 "Don't Touch" / "Hard limits" 명시, (d) reviewer 2-eye 분리(cr + architect).

상세 근거는 `docs/orchestration/CODEX-MCP-PATTERN.md` §3 (B1 결정). `workspace-write` 마이그레이션은 별도 spike SG 없이 시도하지 않는다.

## Iteration cap

`max_review_rounds = 3` (per SG). round 3 에서도 `REQUEST_CHANGES` 면 auto-retry 금지 — operator 에스컬레이션. 상세는 `docs/orchestration/CODEX-MCP-PATTERN.md` §4.

## Reviewer verdict schema

본 dispatcher 가 호출하는 후속 리뷰 prompt 는 다음 verdict enum 만 사용한다:

- **code-reviewer**: `APPROVE | APPROVE_WITH_NIT | REQUEST_CHANGES`
- **architect**: `ARCHITECTURALLY_SOUND | SOUND_WITH_FOLLOWUPS | RECONSIDER`
- **Severity**: `BLOCK | HIGH | MED | LOW | NIT` (`BLOCK/HIGH→fix-now`, `MED+→carry-forward bundle`).

dispatcher 는 envelope 만으로 verdict 를 합성하지 않으며 reviewer agent 호출을 통해서만 verdict 를 받는다.

## Envelope-backup ownership (Phase 7 T1-SG-1, ratified 2026-05-29)

- **Ownership**: envelope-backup 은 orchestrator ratify lane 의 단일 책임이다. 본 dispatcher (= implementer prompt assembler) 는 envelope 을 stdout / Agent driver return value 로만 제공하며, codex worktree commit set 에 envelope 파일을 포함시키지 **않는다**.
- **Canonical path**: `.omx/campaigns/<campaign-id>/envelopes/<sg-id>-<wave-id>.json`. ratify lane 이 ratify commit 직전 단일 step 으로 작성한다.
- **No retroactive**: closed cycle 의 envelope 은 소급 backup 하지 않는다.

상세 5 조항은 `docs/orchestration/CODEX-MCP-PATTERN.md` §9.
...
- commit body 에 why 를 포함했다.
- stdout 마지막 JSON block 이 유효하다.

## L4/L5 governance context in dispatch prompts

orchestrator 가 본 dispatcher 로 prompt 를 조립할 때, Codex implementer 가
governance gate 입력을 **실제 call path 에서** 공급할 수 있도록 다음 context
필드를 prompt 본문에 포함시킨다. 누락 시 mechanism 은 통과해도 live gate 가
dead-rule 로 남는다 (architect D-9 / cr L4/L5 체크리스트 참조).

dispatch prompt 는 상위 **Bet AEC (헌법 2026-06-26 Agent Execution Contract)**
중 이 UoW 에 해당하는 ① Agent Assignment, ② Work Order, ⑤ Done Criteria,
⑥ Acceptance Criteria 를 읽어 in-scope/AC/return-envelope 블록을 구성한다
(UoW 는 Bet 아래 세부 작업계약 — Bet 이 상위 작업지시서). 아래 governance
context 는 그 위에 얹는 gate 입력이다.

- `safety_class` (A/B/C) — risk_level 에서 도출. A=CRITICAL/HIGH, B=MEDIUM,
  C=LOW. Codex 가 verify-단계 `record_evidence` / `verify_dev_session` 호출에
  `typed_verification` flat dict 를 floor 대로 (A={build,test,static,fr_trace,hazard}
  / B={build,test,static} / C={build,test}) 채우게 한다.
- `codex_operating_profile` — Harness C Codex-host/native profile. Per-UoW
  prompt 에 `reasoning_effort`, `verbosity`, preamble, commentary/final phase,
  batched reads, action-gated early stop, conflict priority, and persistence
  boundaries 를 포함시켜 Claude-host delegate 와 Codex-host native 경로가 같은
  운영 계약을 적용하게 한다.
- `lease_file_paths[]` — 편집 대상 파일 목록. Codex 가
  `devos_acquire_fencing_token(resource_kind="file_set", resource_id, holder, ttl_sec, file_paths=[...])`
  으로 lease 를 취득한 뒤 편집을 시작하도록 prompt 에 명시. release 도 명시.
  Bet-level 디스패치(한 Codex 가 Bet 전체 file-set 을 점유)일 때는
  `resource_kind="bet"`(`resource_id=<bet_id>`, `file_paths`=Bet AEC Expected File-set
  = 하위 UoW target_files 합집합) 옵션으로 보강한다(UoW-단위 병렬이면 기존
  `file_set` 옵션 유지 — 둘 다 동일 충돌엔진).
- `prior_art_expectation` — 새 Bet 을 shaping→shaped 로 advance 하는 lane 인
  경우, `create_bet(..., prior_art_refs=[...])` 입력을 prompt 에 명시 (또는
  `problem_statement` lesson recall hit 가능 형태로 작성하도록 지시).
- `origin_ref` — shipped Bet 의 결함 수정 lane 인 경우, reopen 대신
  `create_bet(..., origin_ref="<shipped-bet-id>")` 로 새 Bet 을 만들도록
  명시 (`SHIPPED_REOPEN_FORBIDDEN` 회피).

dispatcher 는 이 context 를 envelope `ac_results` 항목으로도 노출시켜 후속
ratify lane 이 governance gate 발화 여부를 검증할 수 있게 한다.
