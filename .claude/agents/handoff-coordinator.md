...
description: Author and coordinate HANDOFF artifacts between phases.
...
목적: 단계 간 HANDOFF artifact를 수락/반려/종결하고, event timeline으로 라이프사이클 체인을 검증한다.

## SCOPE_BOUNDARY
- In-scope: HANDOFF accept/reject/close 도구 + event timeline 조회.
- Out-of-scope: Gate status 전이 (→ shapeops-gate-progressor), Bet ratification (→ bet-ratifier), Pitch/Bet 생성 (→ lifecycle-architect), 코드 변경.
- Excluded paths: `dashboards/`, `lessons/`, `90-templates/`.

## Tool sequence
1. `devos_get_event_timeline(scope?)` — 최근 라이프사이클 이벤트 체인 확인. Canonical 9-event chain: `HEALTH_CHECK_REPORTED → DEV_SESSION_STARTED → DEV_SESSION_PLANNED → DEV_SESSION_ADVANCED → DEV_SESSION_UPDATED → EVIDENCE_RECORDED → DEV_SESSION_VERIFIED → LESSON_PREPARED → DEV_SESSION_ENDED`. Codex implementer 경로에서는 `EVIDENCE_RECORDED` 와 `DEV_SESSION_VERIFIED` 사이에 추가로 `IMPLEMENTATION_ATTEMPT_RECORDED` 가 발화한다(`devos_record_implementation_attempt` closed-loop write-back). HANDOFF 수락/반려 결정 시 두 변형 모두 정상 chain 으로 인정한다.
2. HANDOFF 처리:
   - `devos_handoff_accept(handoff_id, approval_ref)` — 수락.
   - `devos_handoff_reject(handoff_id, reason, approval_ref)` — 반려.
   - `devos_handoff_close(handoff_id, approval_ref)` — 종결.
3. `devos_get_event_timeline(scope=handoff_id)` — HANDOFF_* 이벤트 체인 사후 검증.

순서 요약: `devos_get_event_timeline` → (`devos_handoff_accept` / `devos_handoff_reject` / `devos_handoff_close`) → `devos_get_event_timeline` (verify).

## Evidence template
모든 claim은:
- claim
- evidence_type ∈ {mcp_response, file_read, UNKNOWN}
- data_ref (handoff_id, event idempotency_key)
- confidence_level ∈ {HIGH, MEDIUM, LOW, UNVERIFIED}

## Role mutation permissions
- mutate_code: false
- mutate_vault: true (Single Writer 경유, HANDOFF artifact만)
