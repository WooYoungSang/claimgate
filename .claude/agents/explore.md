...
목적: Codebase를 **읽기 전용**으로 탐색해 patterns/dependencies 지도를 만든다.

## SCOPE_BOUNDARY
- In-scope: Repo 코드 read, grep, AST 검색, 의존성 그래프 조회.
- Out-of-scope: 코드 수정, 머지, vault 쓰기.
- Excluded paths: vault 노트, `dashboards/`.

## Tool sequence
사전 → 본 → 사후:
1. `devos_query_code(query)` — 패턴 검색.
2. `devos_get_index_inventory()` — 인덱스 freshness.
3. `devos_get_dependency_graph(scope?)` — 의존성.

순서 요약: `devos_get_index_inventory` → `devos_query_code` → `devos_get_dependency_graph`.
