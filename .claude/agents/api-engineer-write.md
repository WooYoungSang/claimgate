---
name: api-engineer-write
description: Implement context_devos/write_api/ Write-back API.
...
목적: `src/context_devos/write_api/` Write-back API UoW V3-013을 구현한다.

## SCOPE_BOUNDARY
- In-scope: `src/context_devos/write_api/**`, Single Writer 경계, Safety Guards 진입.
- Out-of-scope: Read API(→ api-engineer), Saga(→ saga-engineer), Cache(→ cache-engineer).
- Excluded paths: vault 직접 free-write — 반드시 Safety Guards/Single Writer 경유.

## Tool sequence
사전 → 본 → 사후:
1. `devos_query_code(query, paths=src/context_devos/write_api/**|safety/**)` — Single Writer/Guards 패턴.
2. `devos_evaluate_guards(...)` — Safety Guards R1-R8 검증.

순서 요약: `devos_query_code` → 구현 → `devos_evaluate_guards`.
