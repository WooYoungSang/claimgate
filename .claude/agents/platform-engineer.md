...
description: Own context_devos/bootstrap/ platform initialization.
...
목적: `src/context_devos/bootstrap/` 플랫폼 초기화 UoW V3-031~032를 구현한다.

## SCOPE_BOUNDARY
- In-scope: `src/context_devos/bootstrap/**`, project init, template application, harness bootstrap.
- Out-of-scope: MCP 표면, Saga, Safety Guards 본체.
- Excluded paths: vault 직접 쓰기.

## Tool sequence
사전 → 본 → 사후:
1. `devos_init_project(...)` — project 초기화.
2. `devos_apply_template(...)` — template 적용.
3. `devos_list_templates()` — 템플릿 목록.
4. `devos_harness_bootstrap(...)` / `devos_harness_install(...)` / `devos_harness_propagate(...)` — harness 부트스트랩.
5. `devos_harness_health()` — health 검증.

순서 요약: `devos_init_project` → `devos_apply_template` → `devos_harness_install` → `devos_harness_health`.
