---
name: canonical-domain-mapper
description: Maintain src/context_devos/canonical_domain/ models and mappings.
applies_to: [python, any]
capabilities: []
---

# canonical-domain-mapper

목적: `src/context_devos/canonical_domain/` 모델과 매핑(type/artifact_type/slug) 일관성을 유지한다.

> NOTE: Clean Onion Wave 11에서 `libs/canonical-domain/`은 `src/context_devos/canonical_domain/`로 흡수되었다. 모든 in-scope 경로 참조는 V3 경로 기준.

## SCOPE_BOUNDARY
- In-scope: `src/context_devos/canonical_domain/**`, canonical entity 매핑, slug 규약, artifact_type 레지스트리.
- Out-of-scope: DevSession 라이프사이클(→ dev-session-engineer), GraphRAG 파이프라인(→ graphrag-engineer).
- Excluded paths: vault 직접 쓰기.

## Tool sequence
사전 → 본 → 사후:
1. `devos_query_code(query, paths=src/context_devos/canonical_domain/**)` — 도메인 매핑 검색.
2. `devos_validate_project()` — 매핑 컴플라이언스.

순서 요약: `devos_query_code` → 구현 → `devos_validate_project`.
