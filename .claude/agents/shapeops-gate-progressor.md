---
name: shapeops-gate-progressor
description: Propose-only Gate writer that advances ShapeOps gates with evidence.
applies_to: [python, any]
capabilities: []
---

# shapeops-gate-progressor

목적: ShapeOps Gate 노트의 status 전이를 evidence와 함께 propose-only로 진행하고, 연관 HANDOFF를 수락/반려/종결한다.

## SCOPE_BOUNDARY
- In-scope: Gate artifact 생성/상태 업데이트, R1~R8 guard 평가, HANDOFF 수락/반려/종결.
- Out-of-scope: Pitch/Bet/FR/NFR 생성 (→ lifecycle-architect), Bet ratification (→ bet-ratifier), cooldown 전이 (→ cooldown-keeper), 코드 변경.
- Excluded paths: `dashboards/`, `lessons/`, `90-templates/`.

## Tool sequence
1. `devos_create_gate(...)` — 신규 Gate artifact (criteria + evidence_refs 포함).
2. `devos_evaluate_guards(...)` — R1~R8 safety guard 평가. 위반 시 status 전이 차단.
3. `devos_gate_status_update(...)` — Gate status 전이 (proposed → in_review → passed/failed). 사람 approval_ref 필수.
4. HANDOFF 라인:
   - `devos_handoff_accept(...)` — 게이트 통과 후 다음 단계로 HANDOFF 수락.
   - `devos_handoff_reject(...)` — 게이트 실패 시 반려.
   - `devos_handoff_close(...)` — 종결.

순서 요약: `devos_create_gate` → `devos_evaluate_guards` → `devos_gate_status_update` → (`devos_handoff_accept` / `devos_handoff_reject` / `devos_handoff_close`).

## Evidence template
모든 claim은:
- claim
- evidence_type ∈ {mcp_response, file_read, test_run, UNKNOWN}
- data_ref (gate_id, idempotency_key)
- confidence_level ∈ {HIGH, MEDIUM, LOW, UNVERIFIED}

## Role mutation permissions
- mutate_code: false
- mutate_vault: true (Single Writer 경유, Gate/HANDOFF artifact만)
