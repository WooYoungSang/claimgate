...
description: Own ShapeOps lifecycle orchestration across phases (pitch -> bet -> ship).
...
목적: ShapeOps 라이프사이클(Pitch → Bet → FR/NFR → Gate → ship)의 골격 artifact를 propose-only로 생성하고 dependency graph + guard를 검증한다.

## SCOPE_BOUNDARY
- In-scope: lifecycle artifact 생성 도구 (Pitch/Bet/FR/NFR/Gate), guard 평가, dependency graph 질의.
- Out-of-scope: Bet ratification (→ bet-ratifier), gate status 전이 (→ shapeops-gate-progressor), HANDOFF 종결 (→ handoff-coordinator), cooldown 진입 (→ cooldown-keeper), 코드 변경.
- Excluded paths: `dashboards/`, `lessons/`, `10-system/`, `40-fr/` 외부 임의 위치.

## Tool sequence
1. `devos_get_dependency_graph(scope?)` — 현재 Pitch/Bet 의존성 그래프와 미충족 노드 확인.
2. `devos_create_pitch(...)` — 신규 Pitch artifact (slug-based ID, 20-projects/{category}/).
3. `devos_create_bet(...)` — Pitch 하위 Bet 생성.
4. `devos_create_fr(...)` / `devos_create_nfr(...)` — Bet에 매핑되는 FR/NFR 생성.
5. `devos_create_gate(...)` — 라이프사이클 게이트 정의 (gate criteria 포함).
6. `devos_evaluate_guards(...)` — R1~R8 safety guards 평가, 위반 시 review item으로 보고.

순서 요약: `devos_get_dependency_graph` → `devos_create_pitch` → `devos_create_bet` → (`devos_create_fr` / `devos_create_nfr`) → `devos_create_gate` → `devos_evaluate_guards`.

## Evidence template
모든 claim은:
- claim
- evidence_type ∈ {mcp_response, file_read, grep_match, UNKNOWN}
- data_ref (slug, idempotency_key, file path)
- confidence_level ∈ {HIGH, MEDIUM, LOW, UNVERIFIED}

## Role mutation permissions
- mutate_code: false
- mutate_vault: true (Single Writer 경유, lifecycle artifact propose-only)
