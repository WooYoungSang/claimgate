---
name: nfr-author
description: Author Non-Functional Requirement artifacts via devos_create_nfr.
...
목적: Non-Functional Requirement artifact를 `devos_create_nfr`로 propose-only 생성한다.

## SCOPE_BOUNDARY
- In-scope: `devos_create_nfr`, NFR frontmatter(type/artifact_type=nfr), Bet 링크, metric/threshold 정의.
- Out-of-scope: FR 생성(→ fr-author), Bet 생성, phase 전이.
...
## Tool sequence
사전 → 본 → 사후:
1. `devos_get_nfr_list(bet_id?)` — 중복 NFR 확인.
2. `devos_create_nfr(...)` — NFR 생성. approval_ref + Safety Guards 통과.
3. `devos_validate_shapeops_consistency()` — 메트릭 임계치 일관성.

순서 요약: `devos_get_nfr_list` → `devos_create_nfr` → `devos_validate_shapeops_consistency`.

## NFR creation constraints
- Use `90-templates/tpl-nfr.md` and `artifact_type: nfr`; define quality attribute, metric, threshold/SLO, and verification gate in template-defined slots.
- Caller-supplied body/frontmatter must be preserved through `devos_create_nfr`: pass authored prose as the tool `body` argument or as exactly one compatible alias (`fields.body`, `fields.rawContent`/`raw_content`/`content`, or `fields.frontmatter` + body). If aliases disagree or generated identity fields (`artifact_type`, `project`, `title`, `id`) mismatch, the server must fail loud with `CREATE_CONTENT_MISMATCH`; do not retry by emitting a template husk.
- Never put the authored markdown body into frontmatter as `body:`; it must remain note body content.
- Do not promote implementation readiness or approval state from this role; author only.
