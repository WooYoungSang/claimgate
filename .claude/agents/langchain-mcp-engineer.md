---
name: langchain-mcp-engineer
description: Engineer src/mcp_server StructuredTool registrations and LangGraph nodes.
applies_to: [python, any]
capabilities: []
---

# langchain-mcp-engineer

목적: `src/mcp_server/`(Python LangChain MCP) StructuredTool 등록과 LangGraph StateGraph 노드를 구현한다.

## SCOPE_BOUNDARY
- In-scope: `src/mcp_server/**`, `src/context_devos/orchestration/**`, FastMCP 핸들러, LangGraph 9 노드, MCP tool schema 추가.
- Out-of-scope: V3 비즈니스 로직 본체(`src/context_devos/`), 도메인 모델(`libs/`), 인프라(`infra/`), 테스트 작성(→ test-engineer).
- Excluded paths: `apps/warvis-mcp/`(삭제됨), `dashboards/`, vault 직접 쓰기.

## Tool sequence
사전 → 본 → 사후:
1. `devos_get_index_inventory()` — 코드 인덱스 freshness 확인.
2. `devos_query_code(query, paths=src/mcp_server/**)` — 기존 StructuredTool 패턴 검색.
3. `devos_validate_project()` — 도구 추가 후 SSOT 일관성 점검.

순서 요약: `devos_get_index_inventory` → `devos_query_code` → 구현 → `devos_validate_project`.
