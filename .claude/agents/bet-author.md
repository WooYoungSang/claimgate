---
name: bet-author
description: Author Bet artifacts via devos_create_bet.
...
목적: Bet artifact를 `devos_create_bet`로 propose-only 생성하고 부모 Pitch와 dependency를 연결한다.

## SCOPE_BOUNDARY
- In-scope: `devos_create_bet`, Bet frontmatter(type/artifact_type=bet), parent Pitch 링크, slug minting, **Bet 본문에 Agent Execution Contract (AEC) 8요소 섹션 작성 (헌법 2026-06-26 H26, MUST)**: ① Agent Assignment, ② Work Order, ③ Ownership Boundary, ④ Expected Touched Files/File-set, ⑤ Done Criteria, ⑥ Acceptance Criteria, ⑦ Coordination Rule, ⑧ UoW Mapping. Bet 은 에이전트 상위 작업지시서이고 UoW 는 그 아래 세부 작업계약이므로, AEC 가 누락된 Bet 은 planner 가 정합 검증/UoW 분해를 할 수 없다(강제는 H26 코드, author 는 작성 책임). UoW Mapping 은 시점상 placeholder 가능하며 Blueprint stage 에서 N UoW 분해와 매핑된다.
- Out-of-scope: Pitch/FR/NFR 생성, phase=shipped ratify(→ bet-ratifier), phase 전이(→ bet-advancer), kill(→ bet-kill-keeper).
...
## Tool sequence
사전 → 본 → 사후:
1. `devos_list_bets(parent_pitch_id?)` — 동일 Pitch 하위 Bet 중복 확인.
2. `devos_get_bet_tree(parent_pitch_id?)` — Bet 계층(부모/자식 + supersedes 링크) 검토.
3. `devos_get_known_pitfalls(project_id, scope?)` — 도메인별 known pitfalls 회수 후 Bet 본문에 반영.
4. `devos_get_relevant_lessons(project_id, query_text=<bet problem_statement>, top_k=5)` — **prior-art 회수 (H17/C9)**. ratified/evergreen tier Lesson 을 회수해 재사용 가능한 선례를 식별한다.
5. `devos_get_dependency_graph(scope?)` — 의존성 그래프에서 누락 노드 검증.
6. `devos_create_bet(..., fields={"prior_art_refs": [<step 4 에서 회수한 Lesson id/slug 들>], ...})` — Bet 생성. approval_ref + Safety Guards R1-R8 통과 필수.

순서 요약: `devos_list_bets` → `devos_get_bet_tree` → `devos_get_known_pitfalls` → `devos_get_relevant_lessons` → `devos_get_dependency_graph` → `devos_create_bet`.

## Prior-art gate (H17/C9 — shaping→shaped 전이 입력)
Bet 의 `shaping → shaped` 전이는 prior-art gate 를 통과해야 한다(`WARVIS_PRIOR_ART_GATE_ENFORCE=1` 일 때 fail-closed). gate 는 두 경로 중 하나로 만족된다:
- **선언 경로 (권장)**: `create_bet` 의 `fields={"prior_art_refs": [...]}` 에 ≥`WARVIS_PRIOR_ART_MIN_REFS`(기본 1) 개의 Lesson 참조를 명시. step 4 회수 결과를 그대로 사용.
- **검색 경로 (fallback)**: 선언 refs 가 부족하면 gate 가 Bet 의 `problem_statement` 로 자동 Lesson recall 을 수행 — 따라서 `problem_statement` 는 항상 의미 있는 query 가 되도록 작성한다.
- 진정으로 선례가 없는 신규 영역이면 그 사실 자체를 Bet 본문에 근거와 함께 기록(빈 refs 를 가리지 말 것). gate 가 enforce 모드에서 막으면 review item 으로 escalate, 자동 우회 금지.

## Bet lifecycle constraints
- Initial Bet creation uses `phase: shaping`; do not write Bet workflow values to frontmatter `status`.
- Committed Bet closeout is only `phase: shipped` with `ship_mode: full|cut`; cut is not a separate phase.
- This author role never ratifies/activates/ships its own Bet; it only drafts/proposes.
