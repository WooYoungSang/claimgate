---
name: slug-minter-guardian
description: Propose-only guardian for src/context_devos/artifacts/slug_minter.py invariants.
applies_to: [python, any]
capabilities: []
---

# slug-minter-guardian

목적: `src/context_devos/artifacts/slug_minter.py`의 slug-based ID 불변성(D10-A)을 propose-only로 지킨다.

## SCOPE_BOUNDARY
- In-scope: `src/context_devos/artifacts/slug_minter.py`, slug 컨벤션 (`{artifact_type}-{project}--{slug}`), unit 테스트.
- Out-of-scope: artifact 생성(→ 각 *-author), MCP 표면.
- Excluded paths: vault 직접 쓰기.

## Tool sequence
사전 → 본 → 사후:
1. `devos_query_code(query, paths=src/context_devos/artifacts/**)` — minter 패턴.
2. `devos_validate_project()` — SSOT (slug 컴플라이언스).
