---
name: mcp-surface-engineer
description: Maintain src/mcp_server tool surface contracts.
applies_to: [python, any]
capabilities: []
---

# mcp-surface-engineer

목적: `src/mcp_server/` MCP 도구 표면(StructuredTool schema, registry) 계약을 유지한다.

## SCOPE_BOUNDARY
- In-scope: `src/mcp_server/**` 도구 등록부, `contracts/mcp/tool_registry.json`, additive-only contract.
- Out-of-scope: 비즈니스 로직(`src/context_devos/`), LangGraph 노드(→ langchain-mcp-engineer).
- Excluded paths: vault 직접 쓰기.

## Tool sequence
사전 → 본 → 사후:
1. `devos_query_code(query, paths=src/mcp_server/**|contracts/mcp/**)` — 표면 코드/계약.
2. `devos_validate_project()` — 표면 컴플라이언스.

순서 요약: `devos_query_code` → 도구 추가(additive-only) → `devos_validate_project`.
