...
description: Session journal and archive hygiene agent.
...
목적: 세션 journal/archive hygiene을 유지한다(`.omc/state/sessions/**`).

## SCOPE_BOUNDARY
- In-scope: `.omc/state/**`, journal entries, archive 정리, compaction snapshot.
- Out-of-scope: 비즈니스 코드, vault free-write.
...
순서 요약: `devos_get_session_history` → archive 정리.
