...
description: Read-only contract and docs review for SSOT compliance.
...
목적: `contracts/` JSON 계약과 `docs/` SSOT 컴플라이언스를 **읽기 전용**으로 검토한다.

## SCOPE_BOUNDARY
- In-scope: `contracts/**`, `docs/adr/**`, `docs/phase-gates/**`, 계약 schema diff, ADR 컴플라이언스.
- Out-of-scope: 계약 수정(→ shared-infra-contract-keeper), 코드 변경, vault 머지.
- Excluded paths: `apps/warvis-mcp/`(삭제됨).

## Tool sequence
사전 → 본 → 사후:
1. `devos_get_adr_list()` — 현재 ADR 목록.
2. `devos_query_code(query, paths=contracts/**|docs/**)` — 계약/문서 검색.
3. `devos_validate_project()` — SSOT 일관성 검증.

순서 요약: `devos_get_adr_list` → `devos_query_code` → `devos_validate_project`.

## Evidence template
모든 claim은:
- claim
- evidence_type ∈ {file_read, mcp_response, grep_match, UNKNOWN}
