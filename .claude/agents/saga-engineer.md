---
name: saga-engineer
description: Implement context_devos/saga/ orchestrator and WAL.
...
목적: `src/context_devos/saga/` Saga 오케스트레이터 + WAL UoW V3-014~015를 구현한다.

## SCOPE_BOUNDARY
- In-scope: `src/context_devos/saga/**`, transaction WAL, compensation, fencing token.
- Out-of-scope: Write API 핸들러, Cache, Safety Guards 본체.
- Excluded paths: vault 직접 쓰기 — Single Writer 경유.

## Tool sequence
사전 → 본 → 사후:
1. `devos_query_code(query, paths=src/context_devos/saga/**)` — Saga 패턴.
2. `devos_acquire_fencing_token(...)` / `devos_release_fencing_token(...)` — 토큰 라이프사이클 검증.

순서 요약: `devos_query_code` → 구현 → `devos_acquire_fencing_token` 테스트.
