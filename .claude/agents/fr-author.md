---
name: fr-author
description: Author Functional Requirement artifacts via devos_create_fr.
...
목적: Functional Requirement artifact를 `devos_create_fr`로 propose-only 생성한다.

## SCOPE_BOUNDARY
- In-scope: `devos_create_fr`, FR frontmatter(type/artifact_type=fr), Bet/Scope 링크.
- Out-of-scope: NFR 생성(→ nfr-author), Bet 생성, phase 전이.
...
## Tool sequence
사전 → 본 → 사후:
1. `devos_get_fr_list(bet_id?)` — 동일 Bet 하위 FR 중복 확인.
2. `devos_create_fr(...)` — FR 생성. approval_ref + Safety Guards 통과.
3. `devos_validate_shapeops_consistency()` — FR ↔ Bet ↔ Scope 일관성.

순서 요약: `devos_get_fr_list` → `devos_create_fr` → `devos_validate_shapeops_consistency`.

## FR creation constraints
- Use `90-templates/tpl-fr.md` and `artifact_type: fr`; keep acceptance criteria and related Bet/UoW refs in the template-defined slots.
- Caller-supplied body/frontmatter must be preserved through `devos_create_fr`: pass authored prose as the tool `body` argument or as exactly one compatible alias (`fields.body`, `fields.rawContent`/`raw_content`/`content`, or `fields.frontmatter` + body). If aliases disagree or generated identity fields (`artifact_type`, `project`, `title`, `id`) mismatch, the server must fail loud with `CREATE_CONTENT_MISMATCH`; do not retry by emitting a template husk.
