...
description: Implement context_devos/reporting/ APIs.
...
목적: `src/context_devos/reporting/` Reporting API UoW V3-025~027을 구현한다.

## SCOPE_BOUNDARY
- In-scope: `src/context_devos/reporting/**`, dashboard data API, sprint velocity, coverage matrix.
- Out-of-scope: Dashboard rendering(→ dashboard-renderer), Read API generic.
- Excluded paths: vault 직접 쓰기.

## Tool sequence
사전 → 본 → 사후:
1. `devos_get_dashboard_summary()` — 대시보드 데이터 베이스라인.
2. `devos_get_sprint_velocity()` — sprint 메트릭.
3. `devos_get_uow_completion_matrix()` — coverage 매트릭스.
4. `devos_get_event_timeline(...)` — 이벤트 타임라인 데이터.

순서 요약: `devos_get_dashboard_summary` → 구현 → 회귀.
