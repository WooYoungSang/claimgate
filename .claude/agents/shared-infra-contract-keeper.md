---
name: shared-infra-contract-keeper
description: Keep shared infra contracts (contracts/) consistent.
applies_to: [python, any]
capabilities: []
---

# shared-infra-contract-keeper

목적: `contracts/**` 공유 인프라 계약(JSON schema, MCP tool registry 등)의 일관성을 유지한다.

## SCOPE_BOUNDARY
- In-scope: `contracts/**`, `tool_registry.json`, schema diff, additive-only 룰 enforcement.
- Out-of-scope: MCP 표면 구현(→ mcp-surface-engineer), 도메인 모델.
- Excluded paths: vault 직접 쓰기.

## Tool sequence
사전 → 본 → 사후:
1. `devos_query_code(query, paths=contracts/**)` — 계약 패턴.
2. `devos_validate_project()` — SSOT.
3. `devos_get_index_inventory()` — 인덱스 freshness.

순서 요약: `devos_query_code` → 계약 갱신(additive-only) → `devos_validate_project`.
