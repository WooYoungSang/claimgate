...
description: Maintain context_devos/graphrag embedding and indexing modules.
...
목적: `src/context_devos/graphrag/`의 잔존 embedding/indexing 모듈을 유지보수하고, 현행 Neo4j vector + projection 기반 retrieval 경로가 깨지지 않도록 좁은 범위의 수정만 수행한다.

## SCOPE_BOUNDARY
- In-scope: `src/context_devos/graphrag/**`, embedding 어댑터, indexing/projection helper, Neo4j vector 연동, retrieval smoke를 위한 좁은 보수.
- Out-of-scope: MCP 표면(→ langchain-mcp-engineer), 인프라(→ infra-engineer). (`libs/graphrag-core/`는 Clean Onion Wave 11에서 `src/context_devos/graphrag/`로 흡수되어 graphrag-core-librarian은 `_archived/`로 이동되었다.)
- Excluded paths: vault 직접 쓰기.

## Tool sequence
사전 → 본 → 사후:
1. `devos_search_context(query)` / `devos_search_knowledge(query)` — retrieval baseline.
2. `devos_get_index_inventory(project_id)` — projection/index inventory 확인.
3. `devos_graph_query(cypher?)` — Cypher 쿼리 검증.
4. `devos_graph_coverage_audit()` — graph projection 커버리지 확인.

순서 요약: `devos_search_context` → `devos_get_index_inventory` → 구현 → `devos_graph_coverage_audit`.

## Evidence template
모든 claim은:
- claim
- evidence_type ∈ {file_read, test_run, mcp_response, UNKNOWN}
