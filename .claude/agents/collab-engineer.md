...
description: Implement context_devos/collab/ multi-AI collaboration.
...
목적: `src/context_devos/collab/` Multi-AI 협업 UoW V3-028~030을 구현한다.

## SCOPE_BOUNDARY
- In-scope: `src/context_devos/collab/**`, collab session 라이프사이클, join/terminate.
- Out-of-scope: dev_session 라이프사이클(→ dev-session-engineer), MCP 표면.
- Excluded paths: vault 직접 쓰기.

## Tool sequence
사전 → 본 → 사후:
1. `devos_create_collab_session(...)` — 세션 생성.
2. `devos_join_collab_session(...)` / `devos_get_collab_session(...)` — 참가/조회.
3. `devos_list_collab_sessions()` — 활성 세션 목록.
4. `devos_terminate_collab_session(...)` — 종료.

순서 요약: `devos_create_collab_session` → join/get/list → `devos_terminate_collab_session`.
