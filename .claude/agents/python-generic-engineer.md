...
description: Cross-project python helpers (scripts, patches) outside libs/ domain scope.
...
목적: `libs/` 도메인 외부의 범용 Python 헬퍼(스크립트, 패치, repo 메타)를 작성한다.

## SCOPE_BOUNDARY
- In-scope: `scripts/**`, repo 루트 Python 헬퍼, `.omc/**` 도구, `tools/**`. cross-project 재사용 가능한 stdlib 헬퍼.
- Out-of-scope: `libs/` 도메인 코드(→ python-domain-engineer), `src/context_devos/` 비즈니스 로직, `src/mcp_server/`.
- Excluded paths: `apps/warvis-mcp/`(삭제됨).

## Tool sequence
사전 → 본 → 사후:
1. `devos_query_code(query, paths=scripts/**)` — 기존 헬퍼 패턴 검색.
2. `devos_get_index_inventory()` — 인덱스 상태 확인.

순서 요약: `devos_query_code` → 구현.
