---
name: pitch-author
description: Author Pitch artifacts via devos_create_pitch.
...
목적: Pitch artifact를 `devos_create_pitch`로 propose-only 생성하고, slug-based ID와 20-projects 경로를 검증한다.

## SCOPE_BOUNDARY
- In-scope: `devos_create_pitch`, Pitch frontmatter(type/artifact_type=pitch), `20-projects/{category}/` 카테고리 라우팅, slug minting.
- Out-of-scope: Bet/FR/NFR 생성, phase 전이, ratify, gate status 변경, 코드 변경.
- Excluded paths: `dashboards/`, `lessons/`, `10-system/`, review-harness 산출물. `90-templates/` is read-only reference only; mutation prohibited.
...
## Tool sequence
사전 → 본 → 사후:
1. `devos_list_pitches(...)` — 중복/충돌 Pitch 사전 확인.
2. `devos_create_pitch(...)` — Pitch 생성. approval_ref 필요. Safety Guards R1-R8 통과 후에만 vault에 머지.
3. `devos_validate_project()` — 신규 Pitch가 SSOT contract(type+artifact_type, slug ID)를 만족하는지 확인.

순서 요약: `devos_list_pitches` → `devos_create_pitch` → `devos_validate_project`.

## Pitch lifecycle constraints
- Initial Pitch creation uses `phase: shaping`; never set Pitch `phase: shipped`.
- A consumed Pitch exits as `phase: accepted`; explicit human decisions may use `superseded`, `killed`, or `parked`.
- Do not write workflow values to Pitch frontmatter `status`; workflow truth is `phase`.
