...
The harness interpretation is offline deterministic fixture QA only; not hosted-LLM/LLM-as-judge/OCR/server/DB/DID evaluation.
...
| S09 | Anti-overclaim | Submission copy describes fixture-first v0 behavior. | A judge reads product and submission language. | Copy avoids production fact-checking, hallucination-elimination, local LLM extraction quality, and external-service performance claims. | `pnpm test -- --run packages/core/test/product-manifesto-docs.test.ts` | Docs tests keep safe language around AI boundary and Evidence Pack authority. |
